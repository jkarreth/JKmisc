...
function (x, e = 1e-04, mfrow = NULL, include = 1:length(x$namesx), ...) 
{
    plotvar <- function(prob0, mixprobs, means, sds, Emean, Esd, 
        ...
        plot(c(xlower, xupper), c(0, ymax), type = "n", xlab = "", 
            ylab = "", main = name, frame = FALSE, las = 1, cex.axis = 0.5, 
            axes = FALSE)
        axis(2, mgp=c(3, .5, 0))
        axis(1, mgp=c(3, .5, 0))
        grid()
        lines(c(0, 0), c(0, prob0), lty = 1, lwd = 3, lend = 1, ...)
        lines(xx, (1 - prob0) * yy/maxyy, lty = 1, lwd = 1, col = "red", ...)
    }
...
"plot.bicreg" <-
function (x, e = 1e-04, mfrow = NULL, include = 1:x$n.vars, include.intercept = FALSE, ...) 
{
    plotvar <- function(prob0, mixprobs, means, sds, Emean, Esd, 
        name, e = 1e-04, nsteps = 500, ...) {
        if (prob0 == 1) {
